        ...
        request({ method: 'PUT', uri: self.db }, function(err, resp, body){
          if(!err){
            request({ method: 'PUT',
                      ...
                      json: {"views": {"active": {"map": "function(doc) { if(doc.type == \"Timetable\" && doc.valid_until == undefined) emit(doc._id, doc)}" }}}
            }, cb);
          } else { cb() }
        })
      } else { cb() }
...
Client.prototype.get = function(path, callback, type){
  ...
  body_stream.end = function(){
    ...
    self.logger(log, function(doc){
      ...
      if(callback) callback(response.statusCode == 200 ? sizzle.run(dom.document) : undefined, doc || log, body);
    });
  }
};
