...
var Client = exports.Client = function(options){
  ...
  this.logger.create(function(er,ok){
    if(ok){ // push views
      ['Root', 'Timetable'].forEach(function(name){
        var data = fs.readFileSync(__dirname +'/../views/logs/'+ name +'.json', 'utf8').replace(/\n\s*/g, '');
        self.logger.saveDoc('_design/'+ name, JSON.parse(data));
      });
    }
  })
}
...
Client.prototype.getAll = function(){
  ...
  if(self.logger){
    self.logger.view('Root', 'by_age', { limit:1, descending:true, startkey:[self.db.name,{}] }, function(err, resp){
      if(!err && resp.rows.length){
        self.logger.view('Timetable', 'by_age', {descending:true, endkey:[self.db.name, resp.rows[0].key[1]]}, function(err, resp){
