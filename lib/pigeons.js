  ...
  if(self.logger){
    self.logger.view('couchapp', 'runs', { limit:1, descending:true, startkey:[self.db.name,{}] }, function(err, resp){
      if(!err && resp.rows.length){
        self.logger.view('couchapp', 'timetables', {descending:true, startkey:[self.db.name,{}], endkey:[self.db.name, resp.rows[0].key[1]], reduce:false}, function(err, resp){
  ...
  //timetable.on('response', function(response){
    ...
    self.get(uri, function(response, $, html){
      ...
      if(response.statusCode == 200){
        ...
        //if(doc.valid_from != old.valid_from){
...
Client.prototype.parseTimetable = function($) {
  ...
  var scrape = function(arg){
    ...
    var value = $(selector)[0].textContent; //trim
  ...
  //doc['updated_at'] = new Date(); TODO: add database timestamp
