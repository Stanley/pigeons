...
Pigeons.prototype.get = function(uri, callback, type, now){
  var self = this
    , now = now || new Date()
    , body = new Buffer('')
  ...
  req.on('mytimeout', function(){
    console.log("TIMEOUT: "+uri);
    req.abort();
    self.agent = new http.Agent() // http.getAgent(self.server);
    self.get(uri, callback, type, now);
  })
...
Pigeons.prototype.getAll = function(callback){
  ...
  var run = function(err, resp, body){
    ...
    var uri = url.resolve(self.server, self.home)
    self.get(uri, function(window){
      var now = new Date();
      ...
      async.forEachSeries(lines,
        function(link, cb){
          ...
          self.getLine(url.resolve(uri, href), cb)
        },
        function(){
          window.close()
  ...
  this.get(uri, function(window){
    ...
    async.forEachSeries(timetables,
      function(link, cb){
        ...
        that.getTimetable(url.resolve(uri, href), cb);
      },
      function(){
        // Query opposite line
        if(!opposite && opts && opts.opposite && $){
          ...
          async.forEachSeries(lines, function(link, cb){
            that.getLine(url.resolve(uri, link.href), cb, true);
          }, cb);
        } else { cb() }
      }
    );
