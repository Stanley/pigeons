  ...
  async.parallel([
    function(cb){
      if(options.log) {
        ...
        Client.createCouchApp(self.log, cb);
      } else { cb() }
    },
    function(cb){
      // Create db if it doesn't exist
      if(self.db){
        request({ method: 'PUT', uri: self.db }, function(error, resp){
          if(resp.statusCode === 201 || resp.statusCode === 412){ // push views
...
Client.createCouchApp = function(uri, cb){
  couchapp.createApp(require(__dirname +'/../couch/app.js'), uri, function(app){
    app.push(cb) 
  });
}
...
Client.prototype.parseTimetable = function($) {
  ...
  var scrape = function(arg){
    ...
    if(selector.length) {
      ...
      return (regexp ? RegExp(regexp).exec(content)[1] : content).replace(/^\s+|\s+$/g, '');
    } else {
      return undefined;
    }
  }
