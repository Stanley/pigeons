...
Client.prototype.getTimetable = function(uri, link, cb) {
  ...
  timetable.on('response', function(response){
    ...
    response.on('end', function(){
      ...
      var h = d = -1, // indexes of: minutes, hours, days
          days = hours = [],
          horizontal; // Read timetable horizontaly
      
      $(opts.context).forEach(function(el){

        if($.matchesSelector(el, opts.minutes)){
          if(days.length > 1 && horizontal){
            d_ = h%days.length;
          } else {
            d_ = d;
          }
          var minutes = el.textContent.split(" ");
          doc['table'][days[d_]][hours[h]] = 
            doc['table'][days[d_]][hours[h]].concat(minutes.filter(function(m){return m!=''}));
        } else if($.matchesSelector(el, opts.hours)){
          var hour = el.textContent; // parseInt?
          h += 1;
          if(days.length > 1 && horizontal){
            d_ = h%days.length;
          } else {
          ...
          hours.push(hour);
          //sys.puts(d_)
          doc['table'][days[d_]][hour] = [];
        } else if($.matchesSelector(el, opts.days)){
          var day = el.textContent;
          d += 1;
          days.push(day);
          if(h == -1) horizontal = true
